...
public class JelloTouch {
   ...
   public void method13739() {
      ...
      for (Bound bound : this.boundables) {
         if (bound.getKeybindTypes() == KeybindTypes.MODULE && bound.getKeybind() > 0) {
            TouchBarButton var5 = new TouchBarButton();
            var5.setTitle(bound.getModuleTarget().getName());
            var5.setBezelColor(this.method13740(bound.getModuleTarget()));
            var5.setType(TouchBarButton$ButtonType.TOGGLE);
            var5.setAction(new Class2544(this, bound));
            this.field21389.put(bound.getModuleTarget(), var5);
            this.touchBar.addItem(new TouchBarItem(bound.getModuleTarget().getName(), var5, true));
         }
      }
   }
   ...
   public record Class2544(JelloTouch macosTouch, Bound bound) implements TouchBarViewAction {

      @Override
      public void onCall(TouchBarView touchBar) {
         this.bound.getModuleTarget().toggle();
         ((TouchBarButton) touchBar).setBezelColor(this.macosTouch.method13740(this.bound.getModuleTarget()));
      }
   }
}
