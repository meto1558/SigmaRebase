...
public class RotationUtils {
    ...
    public static float[] gcdFix(float[] currentRotation, float[] lastRotation) {
        ...
        float gcd = f * f * f * (bypass ? 8.0F : Client.getInstance().moduleManager.getModuleByClass(AutoSprint.class).getBooleanValueFromSettingName("VulcanGCD") ? 1.2F : 8.0F);

        float deltaYaw = currentYaw - lastYaw;
        float deltaPitch = currentPitch - lastPitch;

 
        float yawOffset = ThreadLocalRandom.current().nextFloat() * 0.001f;
        float pitchOffset = ThreadLocalRandom.current().nextFloat() * 0.001f;
