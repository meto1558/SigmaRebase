...
public class Class9819 {
      ...
      if (this.entity != null) {
         if (this.field45878 != 1) {
            if (this.field45878 == 2) {
               boolean var4 = WorldUtil.isHypixel();
               if (var4) {
                  PlayerAbilities abilities = new PlayerAbilities();
                  abilities.isFlying = true;
                  Entity entity1 = null;

                  for (Entity entity2 : WorldUtil.getEntitesInWorld()) {
                     ...
                     try {
                        ...
                        for (Vector3d vec : vectors) {
                           if (ridingEntity != null) {
                              ridingEntity.getPositionVec().x = vec.getX() + 0.5;
                              ridingEntity.getPositionVec().y = vec.getY();
                              ridingEntity.getPositionVec().z = vec.getZ() + 0.5;
   ...
   public void onMovement(EventMove event) {
      if (this.field45878 == 1) {
         ...
         event.setY(0.0);
         event.setZ(0.0);
         if (this.entity == null || !this.entity.isAlive() || !WorldUtil.getEntitesInWorld().contains(this.entity)) {
            ...
            if (posY < -2.0 && WorldUtil.method17763(this.entity) && this.entity.getPosY() - this.mc.player.getPosY() < -10.0) {
               this.field45878 = 0;
               this.entity = null;
