...
public class KillAuraAttackLambda implements Runnable {
   ...
   private void handleAnimationAndAttack(Minecraft mc, Entity entity, boolean isOnePointEight) {
      ...
      if (attackable || mc.player.isOnGround() && Client.getInstance().moduleManager.getModuleByClass(Criticals.class).isEnabled()) {
   ...
   public void run() {
      ...
      if (!this.killauraModule.getStringSettingValueByName("Attack Mode").equals("Pre")) {
         rayTraceResult = RotationUtil.rayTraceFromPlayer(
         ...
         rayTraceResult = RotationUtil.rayTraceFromPlayer(KillAura.getRotations2(this.killauraModule).yaw, KillAura.getRotations2(this.killauraModule).pitch, range, (double) this.field1477 + motionSpeed);
      }
      ...
      if (var3 && (rayTraceResult != null || !this.killauraModule.getBooleanValueFromSettingName("Raytrace") || mode.equals("Multi"))) {
         for (TimedEntity timedEnt : KillAura.entities) {
            ...
            boolean properTrace = RotationUtil.rayTraceEntity(mc.player, entity);
