...
public class HypixelDisabler extends Module {
    ...
    public void onEnable() {
        if (!mc.player.onGround) {
            ...
            if (!this.getBooleanValueFromSettingName("Instant")) {
                ...
                Client.getInstance().notificationManager.send(new Notification("Hypixel disabler", "Move where you want"));
            }
        }
    }
    ...
    @EventTarget
    public void method16900(SendPacketEvent var1) {
        ...
        if (mc.player != null && this.field23984) {
            if (this.isEnabled() || this.getBooleanValueFromSettingName("Instant")) {
                if (var1.getPacket() instanceof SPlayerPositionLookPacket) {
                    this.access().toggle();
                    if (!this.getBooleanValueFromSettingName("Instant")) {
                        Client.getInstance().notificationManager.send(new Notification("Hypixel disabler", "You can do what you want for 5s"));
                    } else {
                        SPlayerPositionLookPacket var4 = (SPlayerPositionLookPacket) var1.getPacket();
                        var1.cancelled = true;
                        mc.getConnection()
                                .sendPacket(new CPlayerPacket.PositionRotationPacket(var4.getX(), var4.getY(), var4.getZ(), var4.getYaw(), var4.getPitch(), false));
                        ...
                        this.field23983.clear();
                        Client.getInstance().notificationManager.send(new Notification("Hypixel disabler", "Successfully sent packets"));
                    }
